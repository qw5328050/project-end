import axios from 'axios'
import { Message } from 'element-ui'
import store from '@/store'
import { getToken } from '@/utils/auth'
import { handleBlobResponse } from '@/utils/index'

// create an axios instance
const service = axios.create({
  baseURL: process.env.VUE_APP_BASE_API, // url = base url + request url

  // withCredentials: true, // send cookies when cross-domain requests
  timeout: 10000 // request timeout
})

// request interceptor
service.interceptors.request.use(
  (config) => {
    // do something before request is sent

    if (store.getters.token) {
      // let each request carry token
      // ['access_token'] is a custom headers key
      // please modify it according to the actual situation
      config.headers['access_token'] = getToken()
    }

    const code = sessionStorage.getItem('code')
    if (code) config.headers['code'] = code
    config.headers['X-Requested-With'] = 'XMLHttpRequest'
    // config.headers['Source-Url'] = location.origin + '/#/empower'
    config.headers['Source-Url'] =
      location.origin + process.env.BASE_URL + '#/login'
    return config
  },
  (error) => {
    // do something with request error
    console.log(error) // for debug
    return Promise.reject(error)
  }
)

// response interceptor
service.interceptors.response.use(
  /**
   * If you want to get http information such as headers or status
   * Please return  response => response
   */

  /**
   * Determine the request status by custom status
   * Here is just an example
   * You can also judge the status by HTTP Status status
   */
  async(response) => {
    const res = response.data
    if (['blob', 'arraybuffer'].includes(response.config.responseType)) {
      return await handleBlobResponse(response, Message)
    }

    // 2100: 未登录，重定向到登录页
    if (res.status === 2100) {
      store.dispatch('user/resetToken').then(() => {
        window.location.href = res.data
      })
    }

    // if the custom status is not 20000, it is judged as an error.
    if (res.status !== 200) {
      if (res.status === 503) {
        Message({
          message: '服务器正在维护',
          type: 'error',
          duration: 5 * 1000
        })
      } else {
        Message({
          message: res.msg || 'Error',
          type: 'error',
          duration: 5 * 1000
        })
      }
      return Promise.reject(new Error(res.msg || 'Error'))
    } else {
      return res
    }
  },
  (error) => {
    console.log('err' + error) // for debug
    Message({
      message: error.message,
      type: 'error',
      duration: 5 * 1000
    })
    return Promise.reject(error)
  }
)

export default service
