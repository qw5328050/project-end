    ...
    <div class="app-container">
      <el-card class="box-card" style="box-shadow: none">
        ...
        >
          ...
          <el-table-column v-if="form.contractSettlementType!=2" prop="taxIncludeUnitPrice" label="含税单价(元)" />
          <el-table-column v-if="form.contractSettlementType!=2" prop="taxReduceUnitPrice" label="除税单价(元)" />
          ...
          <el-table-column v-if="form.contractSettlementType == 4" prop="renEndDate" label="租赁结束时间" width="200" />
          <el-table-column v-if="form.contractSettlementType == 1" prop="adjustPrice" label="调价(元)" width="120" />
        ...
        >
          ...
          <el-table-column prop="updateTime" label="操作时间">
            <template slot-scope="{row}">
              <div v-if="row.checkStatus !==0" :class="row.isOverFlow == 1 ? 'dateClass' : ''">{{ row.updateTime }}</div>
            </template>
          </el-table-column>
        </el-table>
        <div class="title" style="font-weight: 600">
  ...
  methods: {
    ...
    getDetail(id) {
      ...
      detail(this.$route.query.id, '').then((res) => {
        this.form = { ...res.data }
        if (res.data && res.data.prucReceiptItemPicList.length) {
