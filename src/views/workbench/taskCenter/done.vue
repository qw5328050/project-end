    ...
    <div class="table-box">
      <div class="table-content">
        ...
        >
          ...
          >
            <template slot-scope="{ row }">
              {{ approvalStatus[row.approveStatus] }}
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column key="7" :show-overflow-tooltip="true" width="100" label="操作" fixed="right">
            <template slot-scope="{ row }">
              <el-button
                size="mini"
                type="text"
...
<script>
import { finishedList } from '@/api/waitDeal'
export default {
  name: '',
  filters: {
    numValueIsTrue(val) {
  ...
  methods: {
    ...
    // },
    async searchTodo() {
      var dealList = await finishedList({
        ...this.queryParams,
