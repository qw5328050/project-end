<template>
  <div class="pageWrap">
    ...
    <div v-loading="iframeloading" class="posr ovh" style="height: 315px">
      <iframe
        ref="Iframe"
        :src="whichPlanBasicInfo"
        frameborder="0"
        class="posa mapiframe"
      />
    </div>
        ...
        >
          ...
          <el-card>
            <p
              v-if="index == 0"
              class="init"
              :class="dynamicColor(lineitem.type)"
            />
            <p v-else class="init" :class="dynamicColor(lineitem.type)">
    ...
    <div v-show="moneyShow" class="moneyWrap">
      <el-form :model="regInput">
        <el-form-item
          :rules="[
          ...
          prop="moneyNum"
        >
          <el-input v-model.number="regInput.moneyNum" />
        </el-form-item>
      </el-form>
    </div>
    <template v-if="this.$route.query.show">
      <p class="fz14" style="margin-left: 67px">审批意见</p>
      <el-form ref="dynamicValidateForm" :model="areaForm">
        <el-form-item prop="textarea">
          <el-input
            v-model="areaForm.textarea"
            type="textarea"
            :rows="4"
            class="w50p timeline-textarea"
            placeholder="请输入审批意见，不超过500字符。"
            maxlength="500"
            show-word-limit
          />
        </el-form-item>
      </el-form>
      <div class="mt20" style="margin-left: 67px">
        <el-button
          size="small"
          @click="refuseTask('dynamicValidateForm')"
        >驳回</el-button>
        <el-button
          size="small"
          type="primary"
          @click="permissTask('dynamicValidateForm')"
        >同意</el-button>
      </div>
    </template>
  </div>
...
export default {
  ...
  async created() {
    ...
    if(params.processCode === 'processSettlementTotal' && params.taskDefKey === 'companyMaterialManager') {
      this.numberLabel = '实际结算金额'
    } else {
      this.numberLabel = '累计已付金额'
    }
