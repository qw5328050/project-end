  ...
  >
    ...
    >
      ...
      <el-table-column prop="specification" label="规格型号(工料机描述)">
        <template slot-scope="{ row }">
          <span v-if="row.remarks">{{ row.specification || '-' }} - {{ row.remarks || '' }}</span>
      ...
      <el-table-column prop="availableQuantity" align="center" label="可用数量">
        <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      selectByplanId({ planId: this.totalId })
        .then((res) => {
          this.list = res.data
          this.list.forEach((item) => {
            if (item.id === this.id && this.selectData.length === 0) {
