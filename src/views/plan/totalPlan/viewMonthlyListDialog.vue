<template>
  ...
  >
      ...
      <el-row :gutter="20">
        <el-col :span="9">
          <el-form-item label="月度计划名称:" prop="monthPlanName">
            <el-input v-model="queryParams.monthPlanName" />
          </el-form-item>
        </el-col>
        <el-col :span="9">
    ...
    >
      ...
      <el-table-column label="月度计划编号" prop="monthPlanCode">
        <template slot-scope="{ row }">
          {{ row.monthPlanCode || '-' }}
        </template>
      </el-table-column>
...
import { getList } from '@/api/plan/monthlyPlan'
export default {
  ...
  methods: {
    getList() {
      // 请求数据列表
      this.loading = true
      this.queryParams.overallPlanId = this.id
