            ...
            <div class="table_div">
              ...
              >
                ...
                >
                  <template slot-scope="{ row, $index }">
                    <el-form-item
                      :prop="`list[${$index}]` + '.meteringUnits'"
                      :rules="tableForm.rules.meteringUnits"
                    >
                      <el-input
                ...
                >
                  <template slot-scope="{ row, $index }">
                    <el-form-item
                      :prop="'list.' + $index + '.steelType'"
                      :rules="tableForm.rules.steelType"
                    >
                      <el-select
                        v-model="row.steelType"
                        filterable
                        :disabled="row.isEdit"
                        clearable
                      >
                        <el-option
                          v-for="item in getDictionaryValue('steelType')"
                          :key="item.value"
                          :label="item.label"
                          :value="item.value"
                        />
  ...
  methods: {
    ...
    handleUploadSuccess(res, file) {
      if (res.success) {
        if (this.tableForm.list && res.data && res.data.length > 0) {
          ...
          const arr = this.tableForm.list.filter(item => item.id)
          this.tableForm.list = arr.concat(res.data)
        }
      } else {
    ...
    getPlanDetail(id) {
      getDetailPlan({ planId: id }).then((res) => {
        ...
        this.oldList = res.data
          ? JSON.parse(JSON.stringify(res.data.materialList))
          : []
    ...
    handlerShwoNode() {
      ...
      if (this.tableForm.list && this.tableForm.list.length > 0) {
        ...
        try {
          this.tableForm.list.forEach((item) => {
            const obj = this.oldList.find((items) => items.id === item.id)
            if (obj) {
              if (obj.totalQuantity > item.totalQuantity) {
                errorData = item
                throw new Error()
              }
            }
          ...
          this.$message({
            message: `物资名称${errorData.materialName},使用部位${errorData.usePosition}这条数量填写有误,数量不可小于之前的量`,
            type: 'warning'
          })
          return
        }
    ...
    // 新增行
    addTable() {
      this.tableForm.list.push({ isEdit: false, remarks: '' })
    },
    // 复制行
    handelCopy(row) {
      const obj = { ...row }
      delete obj.id
      this.tableForm.list.push({ ...obj, isEdit: false })
    },
    // 删除行
