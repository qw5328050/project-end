<template>
  <div class="totalPlan_container">
    ...
    <div class="table_box">
      <div class="table_content">
        <div class="table_header">
          <div class="table_title">总计划列表</div>
          <div class="table_button">
            <el-button
              v-hasPermi="[
                'xm:menu:bs:material:plan_manage:master_plan:print',
                'isAll'
              ]"
              @click="handlerPrint"
            >
            ...
            <el-button
              v-hasPermi="[
                'xm:menu:bs:material:plan_manage:master_plan:material_export',
                'isAll'
              ]"
              @click="exportData(2)"
            >
            ...
            <el-dropdown style="margin-left: 10px" @command="toAddView">
              <el-button
                v-hasPermi="['xm:menu:bs:material:plan_manage:master_plan:add']"
                type="primary"
                icon="el-icon-plus"
              >新增</el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  command="/plan/materialsNeeded?type=1"
                >物资需用总计划</el-dropdown-item>
                <el-dropdown-item
                  command="/plan/reinforcedNeeded?type=2"
                >钢筋需用总计划</el-dropdown-item>
                <el-dropdown-item
                  command="/plan/concreteNeeded?type=3"
                >混凝土需用总计划</el-dropdown-item>
                <el-dropdown-item
                  command="/plan/returnableNeeded?type=4"
                >设备/周转材料/加工定制产品需用总计划</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
            <right-toolbar
              :size="size"
        ...
        >
          ...
          >
            <template slot-scope="{ row }">
              <template v-if="row.planStatus === 5 || row.planStatus === 7">
                <el-button
                  v-hasPermi="[
                    'xm:menu:bs:material:plan_manage:master_plan:edit'
                  ]"
                  size="mini"
                  type="text"
                  @click="handlerAlteration(row)"
                >编辑</el-button>
              </template>
              <template
                v-if="
              ...
              <el-popconfirm
                ...
                @onConfirm="handlerdDeletePlan(row)"
              >
                <el-button
                  slot="reference"
                  v-hasPermi="[
                    'xm:menu:bs:material:plan_manage:master_plan:delete'
                  ]"
                  size="mini"
                  type="text"
                >删除</el-button>
              ...
              <el-button
                v-show="row.planStatus === 3"
                v-hasPermi="[
                  'xm:menu:bs:material:plan_manage:master_plan:change'
                ]"
                size="mini"
                type="text"
                @click="handlerAlteration(row)"
              ...
              <el-button
                v-show="row.planStatus === 3"
                v-hasPermi="[
                  'xm:menu:bs:material:plan_manage:master_plan:start_monthly_plan'
                ]"
                size="mini"
                type="text"
                @click="initiateMonthlyPlan(row)"
              >发起月度计划</el-button>
              <!-- <el-button
                v-if="row.planStatus === 2"
                v-hasPermi="['plan:overall:approvalRecord']"
                size="mini"
                type="text"
              ...
              <el-button
                ...
                v-hasPermi="[
                  'xm:menu:bs:material:plan_manage:master_plan:approval_progress',
                  'isAll'
                ]"
                size="mini"
                type="text"
    ...
    <ViewMonthlyListDialog
      :id="totalId"
      :visible.sync="isShowMonthlyDialog"
      title="查看月度关联计划"
    />
    <ApprovalProgress
    ...
    >
      <template #content="{ row, list }">
        <template v-if="row.planType === 1 || row.planType === 4">
          <tr class="text-c">
            <td colspan="1">序号</td>
            <td colspan="1">使用部位</td>
            <td colspan="1">物资名称</td>
            <td colspan="5">规格型号(工料机描述)</td>
            <td colspan="2">工料机补充描述</td>
            <td colspan="1">计量单位</td>
            <td colspan="1">数量</td>
          </tr>
          <tr v-for="(item, index) in list" :key="index" class="text-c">
            <td colspan="1">{{ index + 1 }}</td>
            <td colspan="1">{{ item.usePosition }}</td>
            <td colspan="1">{{ item.materialName }}</td>
            <td colspan="5">{{ item.specification }}</td>
            <td colspan="2">{{ item.remarks }}</td>
            <td colspan="1">{{ item.meteringUnits }}</td>
            <td colspan="1">{{ item.totalQuantity }}</td>
          </tr>
        </template>
        <template v-if="row.planType === 2">
          <tr class="text-c">
            <td rowspan="2" :colspan="row.base">楼层名称</td>
            <td rowspan="2" :colspan="row.base">钢筋总重</td>
            <td
              v-if="row.oneColum.length > 0"
              :colspan="row.oneColum.length * row.base"
            >
              一级钢
            </td>
            <td
              v-if="row.threeColum.length > 0"
              :colspan="row.threeColum.length * row.base"
            >
              三级钢
            </td>
            <td
              v-if="row.fourColum.length > 0"
              :colspan="row.fourColum.length * row.base"
            >
              四级钢
            </td>
          </tr>
          <tr class="text-c">
            <template v-if="row.oneColum">
              <td
                v-for="itemClom in row.oneColum"
                :key="itemClom.lable"
                :colspan="row.base"
              >
                {{ itemClom.lable }}
            ...
            <template v-if="row.threeColum">
              <td
                v-for="itemClom in row.threeColum"
                :key="itemClom.lable"
                :colspan="row.base"
              >
                {{ itemClom.lable }}
            ...
            <template v-if="row.fourColum">
              <td
                v-for="itemClom in row.fourColum"
                :key="itemClom.lable"
                :colspan="row.base"
              >
                {{ itemClom.lable }}
          ...
          <tr
            ...
            :key="index + 'item' + '121212'"
            class="text-c"
          >
            <td
              :colspan="row.base"
              :style="{ width: 100 / row.columnTotal + '%' }"
            >
              {{ item.usePosition }}
            </td>
            <td
              ...
              :style="{ width: 100 / row.columnTotal + '%' }"
            >
              {{ item.reinforcedTotalWeight }}
            </td>
            <template v-if="row.oneColum">
              <td
                v-for="itemClom in row.oneColum"
                :key="itemClom.lable + 'oneColum' + index"
                :colspan="row.base"
                :style="{ width: 100 / row.columnTotal + '%' }"
              >
                {{ item[itemClom.value] }}
            ...
            <template v-if="row.threeColum">
              <td
                v-for="itemClom in row.threeColum"
                :key="itemClom.lable + 'threeColum' + index"
                :colspan="row.base"
                :style="{ width: 100 / row.columnTotal + '%' }"
              >
                {{ item[itemClom.value] }}
            ...
            <template v-if="row.fourColum">
              <td
                v-for="itemClom in row.fourColum"
                :key="itemClom.lable + 'fourColum' + index"
                :colspan="row.base"
                :style="{ width: 100 / row.columnTotal + '%' }"
              >
                {{ item[itemClom.value] }}
        ...
        <template v-if="row.planType === 3">
          <tr class="text-c">
            <td rowspan="2" :colspan="row.base">楼层名称</td>
            <td rowspan="2" :colspan="row.base">混凝土总重/m³</td>
            <td
              v-if="row.labelColum"
              :colspan="row.labelColum.length * row.base"
            >
              标号
            </td>
          </tr>
          <tr class="text-c">
            <template v-if="row.labelColum">
              <td
                v-for="itemClom in row.labelColum"
                :key="itemClom.lable"
                :colspan="row.base"
              >
                {{ itemClom.lable }}
          ...
          >
            ...
            <template v-if="row.labelColum.length > 0">
              <td
                v-for="itemClom in row.labelColum"
                :key="itemClom.lable + 'labelColum' + index"
                :colspan="row.base"
                :style="{ width: 100 / row.columnTotal + '%' }"
              >
                {{ item[itemClom.value] }}
...
import ViewMonthlyListDialog from './viewMonthlyListDialog.vue'
import {
  getList,
  deletePlan,
  exportTotalPlan,
  ...
  methods: {
    ...
    handlerPrint() {
      if (this.ids && this.ids.length > 0) {
        ...
        this.$message.warning('请先选择需要打印的总计划')
      }
    },
    changeArr(arr) {
      const idArr = []
            ...
            item.materialList.forEach(materialItem => {
              if (item.planType === 2 || item.planType === 3) {
                if (materialItem.remarks) {
                  materialItem.specification = materialItem.specification + '-' + materialItem.remarks
                }
              }
            })
          }
            ...
            approvalProgress({
              ...
              if (res.data && res.data.length > 0) {
                ...
                arrData.forEach((resDataItem, index) => {
                  if (index === 0) {
                    this.$set(item, 'tabulation', resDataItem.assigneeName)
                    this.$set(item, 'tabulationTime', resDataItem.createTime)
                  } else if (index === 1) {
                    this.$set(item, 'audit', resDataItem.assigneeName)
                    this.$set(item, 'auditTime', resDataItem.createTime)
                  } else if (index === 2) {
                    this.$set(item, 'approval', resDataItem.assigneeName)
                    this.$set(item, 'approvalTime', resDataItem.createTime)
                  }
                })
              }
              res.data
