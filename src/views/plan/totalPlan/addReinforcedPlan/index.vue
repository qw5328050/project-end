              ...
              >
                ...
                >
                  <template slot-scope="{ row, $index }">
                    ...
                    >
                      <el-select v-model="row.steelType" filterable clearable>
                        <el-option
...
export default {
  ...
  methods: {
    ...
    demandSubtotal(row) {
      ...
      const mainBody = Number(row.mainBody) || 0
      const renovate = Number(row.renovate) || 0
      const other = Number(row.other) || 0
      row.subtotal = basics + mainBody + renovate
      row.totalQuantity = row.subtotal + other
    },
    changeTotalQuantity(row) {
      const subtotal = Number(row.subtotal) || 0
      const other = Number(row.other) || 0
      row.totalQuantity = subtotal + other
    }
  }
}
</script>
