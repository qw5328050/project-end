      ...
      >
        ...
        >
          ...
          <el-table-column label="暂估金额">
            <template slot-scope="{ row, $index }">
              <el-form-item :prop="`reportList.${$index}.totalAmount`" :rules="rules.totalAmount">
                <el-input v-model="row.totalAmount" maxlength="10" />
              </el-form-item>
            </template>
          </el-table-column>
...
import {wasteAdd,wasteDetail } from '@/api/wasteMange/wasteMange'
export default {
  name: "transferAsk",
  components: {
    SelectNode,
  ...
  methods: {
    ...
    submit(dataArr) {
      if (dataArr.length >= 1) {
        dataArr.forEach((item, index) => {
          ...
          obj.specification=item.specifications
          obj.unit=item.unit
    ...
    async commonSubmit(submitStatus) {
      ...
      formData.wasteApply= this.form
      formData.materialFiles = submitFileList
      // 底部table
      formData.wasteApplyItems = this.form.reportList

      if (this.pageType === 'edit') {
        formData.id = this.$route.query.id
        const res = await wasteAdd(formData)
