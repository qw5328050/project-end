        ...
        <div class="table-header">
          ...
          <div class="table-col">
            ...
            <right-toolbar
              v-show="table"
              :columns="preColumns"
              @queryTable="getPreOrderList"
              @resetColumns="resetPreColumns"
              @changeSize="changeSize"
            />
            <right-toolbar
              v-show="!table"
              :columns="columns"
              @queryTable="getNormalOrderList"
          ...
          >
            ...
            <el-table-column key="14" label="操作" width="200">
              <template slot-scope="{ row, $index }">
                <el-button
                  v-show="row.orderStatus === 1 || row.orderStatus === 4 || row.orderStatus === 5"
                ...
                >编辑</el-button>
                <el-popconfirm
                  v-if="row.orderStatus === 1||row.orderStatus=== 4 || row.orderStatus === 5"
                  title="确定要删除这一行吗？"
...
import indexDialog from '@/views/purchase/dispatch/indexDialog.vue'
import printOrder from '@/views/purchase/printOrder/index.vue'
...
import selectSupplier from '@/views/purchase/order/selectSupplier'
...
export default {
  ...
  methods: {
    ...
    getNormalOrderList() {
      this.loading = true
      getNormalOrderList({ ...this.normalOrderParam, ...this.projectInformation, platFormType: 1 })
        .then((res) => {
    ...
    edit(row) {
      ...
      this.$router.push({
        path: '/purchase/edit',
        query: { id: row.id, clickStatus: clickStatus }
      })
    },
    // 下单跳转页面
    orders(row) {
      this.$router.push({
        path: '/purchaseAsset/placeOrder',
        query: { id: row.id }
      })
    },
    // 更改表格密度
