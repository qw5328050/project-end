          ...
          <el-table-column width="200" label="操作" fixed="right">
            <template slot-scope="{ row }">
              <el-button
                ...
                >编辑</el-button
              >
              <el-button v-if="row.status === 2 || row.status === 3" size="mini" type="text" @click="toDetails(row)"
                >查看审批详情</el-button
              >
              <el-button size="mini" v-if="row.status === 2" type="text" @click="retreatProcess(row)"
                >撤回</el-button
              >
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toAddPage(row = null) {
      this.$router.push({
        path: "/materialsSiteManagement/planAdd",
        query: {
          id: row ? row.id : "",
        },
      });
    ...
    toDetails(row) {
      this.$router.push({
        ...
        query: { id: row.id, processInstanceId: row.processInstanceId },
      });
    },
    // 多选
