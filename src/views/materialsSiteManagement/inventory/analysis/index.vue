...
import { getList } from "@/api/materialsSiteManagement/analysis";
...
export default {
  ...
  methods: {
    ...
    toAddPage(row = null) {
      this.$router.push({
        path: "/materialsSiteManagement/analysisAdd",
        query: {
          id: row ? row.id : "",
        },
      });
    ...
    toDetails(row) {
      this.$router.push({
        path: "/materialsSiteManagement/analysisDetails",
        query: { id: row.id },
      });
    },
    // 多选
        ...
        .then((res) => {
          ...
          list.forEach(item => {
            if (item.operatorDTOS?.length) {
              const operatorString = item.operatorDTOS.map(item => item.operatorUserName).join('、')
