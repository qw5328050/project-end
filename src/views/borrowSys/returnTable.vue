  ...
  <div class="totalPlan-container">
    <el-card class="pm">
      ...
      >
        ...
        <el-row>
          ...
          <el-col :xl="7" :lg="8" :xs="12" :sm="12" :md="12">
            <el-form-item label="原因" prop="reason" :rules="form.rules.reason">
              <el-input v-model="form.reason" placeholder="请输入" style="width: 80%" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <AllDelete @isCheck="isCheck" @del="del" :checked="checked" :sel-num="selNum"/>
        <el-table
          ref="multipleTable"
          ...
          <el-table-column prop="num" label="退料数量">
            <template slot-scope="{ row }">
              <el-input
                  ...
                  clearable
                  :disabled="pageType == 'stop'"
                />
            </template>
          </el-table-column>
          <el-table-column prop="unit" label="单位"></el-table-column>
          <el-table-column prop="taxIncludeUnitPrice" label="含税单价(元)"></el-table-column>
          <el-table-column prop="taxReduceUnitPrice" label="除税单价(元)"></el-table-column>
...
import { toReturnDetail, toPrucReturnByReportStopType, toPrucReturn, stopDetail, selectUsersByRole} from '@/api/borrowSys/prucReportStop'
// import {
...
export default {
  ...
  methods: {
    initPage() {
      if (this.$route.query.pageType == 'lease') {
        let {id,receiptCode,receiptItemId} = this.$route.query
    ...
    async selectUsersByRole (projectId) {
      let res = await selectUsersByRole({ id: projectId })
    ...
    showDetail(){
      ...
      stopDetail(parm).then(res=>{
        if(res.status == 200){
          ...
          const {receiptCode, returnCode} = res.data
          let result = res.data
          ...
          this.form.prucReturnMaterialDTOList = toReportStopDetailMateiralVoList
          this.selectUsersByRole(result.projectId)
        }
      }).catch(err=>{
        console.log(err)
      })
    ...
    async getDetail() {
      this.loading = true;
      let params = {
        reportStopReturnIdList: Array.isArray(this.$route.query.id) ?  this.$route.query.id : [this.$route.query.id]
      }
      let res =  await toReturnDetail(params)
      if (res && res.status == 200) {
        let result = res.data || {}
        this.loading = false
        this.form = Object.assign(this.form, result, result.consigneeInformationVo)
        this.form.prucReturnMaterialDTOList = result.toReportStopDetailMateiralVoList
        delete this.form.toReportStopDetailMateiralVoList
        this.selectUsersByRole(result.projectId)
      }
    },
    // 选择供应商弹窗
    ...
    // 提交按钮
    submitForm(type) {
      let reportStopId = this.$route.query.reportStopId
      ...
      this.$refs["form"].validate(async (valid) => {
        if (valid) {
          ...
          if (this.$route.query.pageType == 'lease') {
            ...
            res = await toPrucReturn(this.form)
          } else {
            this.form.reportStopIdList = Array.isArray(reportStopId) ? reportStopId : [reportStopId]
            res = await toPrucReturnByReportStopType(this.form)
          }
          if (res && res.status == 200) {
