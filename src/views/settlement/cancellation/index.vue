    ...
    <div class="table-box">
      <div class="table-content">
        ...
        >
          ...
          <el-table-column label="物资种类" prop="type" show-overflow-tooltip>
        ...
        <div class="pagination">
          ...
          <Pagination
            ...
            @pagination="listAdjustmentByTotalOrder"
          />
        </div>
      </div>
    </div>
...
<script>
import { listAdjustmentByTotalOrder, saveReopenOrder } from '@/api/lumpsum/test'
...
export default {
  ...
  mounted() {
    ...
    var id = this.$route.query.id.toString()
    this.$set(this.queryParams, 'orderIds', id)
  },
  created() {
    this.listAdjustmentByTotalOrder()
  },
  methods: {
    back() {
      this.$router.push({
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          that.$confirm('是否确认作废重开收料单?', '警告', {
            ...
            .then(() => {
              that.from.orderNo = that.$route.query.totalNo // 上个页面传的结算单的id
              ...
              saveReopenOrder(that.from).then((res) => {
                that.$message('操作成功')
                that.$router.push({
                  path: '/settlement/lumpsum',
                  query: { activeName: 'second' }
