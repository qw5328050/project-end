<template>
  <div>
    ...
    >
      ...
      <el-table-column label="含税调价" prop="includeTaxAdjustPrice">
        <template slot-scope="{ row }">
          <div>{{ row.includeTaxAdjustPrice || 0 }}</div>
        </template>
      </el-table-column>
      <el-table-column label="含税附加费" prop="includeTaxExtraPrice">
        <template slot-scope="{ row }">
          <div>{{ row.includeTaxExtraPrice || 0 }}</div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    calculate() {
      if (this.type === 1) {
        this.itemVoList = this.list.generalItemVoList
        this.show = false // 不能编辑
        this.$emit('submitForm', this.list)
      }
      if (this.type === 2) {
        this.itemVoList = this.list.generalVoList
        this.show = true // 能编辑
      }
      if (this.type === 3) {
        this.itemVoList = this.list.generalVoList
