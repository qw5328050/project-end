  ...
  <table class="table table-bordered" style="page-break-after: always">
    <tbody>
      <tr class="text-c">
        ...
        <td colspan="15" style="font-size: 24px">中国建筑项目管理表格</td>
      </tr>
      <tr class="text-c header_col">
        <td rowspan="2" colspan="9" style="font-size: 24px">
          {{ getLable(printData.type) || '' }}
        </td>
        <td colspan="6">表格编号</td>
      </tr>
      <tr class="text-c header_col">
        <td v-if="printData.type === 1" colspan="6">CSCEC8BNF-MM-B10131</td>
        <td v-if="printData.type === 3" colspan="6">CSCEC8BNF-MM-B10132</td>
        <td v-if="printData.type === 4" colspan="6">CSCEC8B-PS-B30903</td>
        <td v-if="printData.type === 5" colspan="6">CSCEC8B-PS-B309021</td>
      </tr>
      <tr class="text-c">
        <td colspan="2">项目名称</td>
        <td colspan="6">{{ printData.projectName || '' }}</td>
        <td colspan="2">日期</td>
        <td colspan="6">{{ printData.settlementDate || '' }}</td>
        <td colspan="2">单位：元</td>
      </tr>
      <tr class="text-c">
        <td colspan="2">供应商</td>
        <td colspan="7">{{ printData.supplierName || '' }}</td>
        <td colspan="2">合同编号</td>
        <td colspan="7">{{ printData.contractCode || '' }}</td>
      </tr>
      <tr class="text-c">
        <td colspan="2">结算编号</td>
        <td colspan="16">{{ printData.orderNo || '' }}</td>
      </tr>
      <tr class="text-c">
        ...
        <td colspan="1" rowspan="2">浮动比例</td>
        <td colspan="1" rowspan="2">合同约定浮动价</td>
      ...
      <template v-if="printData.type === 1">
        <tr
          v-for="(item, index) in printData.rebarItemVoList"
          :key="index"
          class="text-c"
        >
          ...
          <td colspan="1">{{ item.floatingScale || '0' }}%</td>
          <td colspan="1">
            {{ item.excludeTaxContractAppointmentPrice || '0' }}
          </td>
      ...
      <template v-if="printData.type === 3">
        <tr
          v-for="(item, index) in printData.floatingItemVoList"
          :key="index"
          class="text-c"
        >
          <td colspan="1">{{ index + 1 }}</td>
      ...
      <template v-if="printData.type === 4">
        <tr
          v-for="(item, index) in printData.netPriceItemVoList"
          :key="index"
          class="text-c"
        >
          <td colspan="1">{{ index + 1 }}</td>
      ...
      <template v-if="printData.type === 5">
        <tr
          v-for="(item, index) in printData.concreteItemVoList"
          :key="index"
          class="text-c"
        >
          <td colspan="1">{{ index + 1 }}</td>
      ...
      <tr class="text-c">
        ...
        <td colspan="1">
          {{
            printData.totalOtherAmounts - printData.totalOtherAmountsNoTax ||
              '0'
          }}
        </td>
      ...
      <tr class="text-c">
        ...
        <td colspan="4" />
        <td colspan="1">{{ printData.settlementAmtNoTax || '0' }}</td>
      ...
      <tr class="text-c">
        ...
        <td colspan="8">{{ printData.settlementAmtNoTax || '0' }}</td>
        <td colspan="1">人民币大写:</td>
        <td colspan="7">{{ printData.settlementAmtNoTaxWords || '' }}</td>
      </tr>
      <tr class="text-c">
        <td colspan="2" class="text-c">本次合计（含税）</td>
        <td colspan="8">{{ printData.settlementAmtTax || '0' }}</td>
        <td colspan="1">人民币大写:</td>
        <td colspan="7">{{ printData.settlementAmtTaxWords || '' }}</td>
      </tr>
      <tr class="text-c">
        <td colspan="2" class="text-c">累计发生（不含税）</td>
        <td colspan="8">{{ printData.cumSettlementAmtNoTax || '0' }}</td>
        <td colspan="1">人民币大写:</td>
        <td colspan="7">{{ printData.cumSettlementAmtNoTaxWords || '' }}</td>
      </tr>
      <tr class="text-c">
        <td colspan="2" class="text-c">累计发生（含税）</td>
        <td colspan="8">{{ printData.cumSettlementAmtTax || '0' }}</td>
        <td colspan="1">人民币大写:</td>
        <td colspan="7">{{ printData.cumSettlementAmtTaxWords || '' }}</td>
      </tr>
      <tr class="text-c">
        <td colspan="18" class="content">
      ...
      <tr class="text-c">
        ...
        <td colspan="4">{{ printData.materialEngineerTime || '' }}</td>
      </tr>
      <tr class="text-c">
